...
Construct clickable Cubox links from any resource ID (card, folder, tag). No API call needed — just the ID + server. **[Must-read: Deep Links](references/deep-links.md)**.

Default: `https://{server}/web/card/{ID}` — use `cubox://` only when explicitly requested.

## Time filtering

`--start-time` and `--end-time` accept flexible shorthand values:
- `--start-time` → start of day (00:00:00.000)
- `--end-time` → end of day (23:59:59.999)

Accepted formats: `today`, `yesterday`, `now`, `7d`, `2026-01-01`, `2026-01-01 15:04:05`, full ISO timestamp.
...
cubox-cli automatically checks for new versions. JSON output may include `_notice.update`:
...
**When you see `_notice.update`, do NOT silently ignore it.** Tell the user an update is available with the versions and the command:

```bash
npm update -g cubox-cli && npx skills add OLCUBO/cubox-cli -g -y
```

After updating, remind the user to **exit and reopen the AI Agent** so the latest Skill is loaded.
...
- Do not silently ignore `_notice.update`. After completing the user's current request, proactively mention the available update.
